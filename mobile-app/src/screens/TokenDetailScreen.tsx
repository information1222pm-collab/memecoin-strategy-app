...
const TokenDetailScreen = ({ route }: Props) => {
  ...
  const handleTrade = async () => {
    ...
    setIsTrading(true);
    try {
        // In a real app, you would get amount from user input
  ...
  const ChartComponent = () => {
    ...
    return (
        <View style={styles.chartContainer}>
            <LineChart
                data={currentChart}
                width={isTabletLayout ? (width * 0.55) : width - 32}
                height={280}
                chartConfig={{
  ...
  // This is the correct return statement that includes all components
  return (
...
const styles = StyleSheet.create({
  ...
  tradeSection: { marginHorizontal: 16, marginTop: 32, marginBottom: 48, padding: 16, backgroundColor: colors.card, borderRadius: 8, borderWidth: 1, borderColor: colors.border },
